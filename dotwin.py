...
def process_data_in_chunks(file_path, chunk_size=100000):
    ...
    for chunk in pd.read_csv(file_path, chunksize=chunk_size):
        ...
        # Step 1: Clean the chunk (remove NaN or empty content)
        chunk_cleaned = chunk.dropna(subset=['content'], how='all')  # Drop rows if 'content' is NaN
        chunk_cleaned = chunk_cleaned[(chunk_cleaned['content'].str.strip() != '')]  # Keep rows with non-empty 'content'
        
        # Step 2: Remove rows with links in 'content'
        chunk_cleaned = chunk_cleaned[~chunk_cleaned['content'].str.contains(r'http[s]?://\S+', na=False)]
...
# Process the dataset in chunks of 100k rows from the CSV file
processed_df = process_data_in_chunks("../largedata/new_range/dotwin_comments.csv", chunk_size=100000)
