...
# Function to process and clean data in chunks
def process_data_in_chunks(file_path, chunk_size=100000):
    final_results = []  # This will accumulate all results
    chunk_count = 0
    
    for chunk in pd.read_csv(file_path, usecols=['body'], chunksize=chunk_size):  # Only read the 'body' column
        chunk_count += 1
        print(f"Processing chunk {chunk_count}...")
        
        # Step 1: Clean the chunk (remove NaN or empty 'body')
        chunk_cleaned = chunk.dropna(subset=['body'], how='all')  # Drop rows if 'body' is NaN
        chunk_cleaned = chunk_cleaned[(chunk_cleaned['body'].str.strip() != '')]  # Keep rows with non-empty 'body'
        
        # Step 2: Remove rows with links in 'body'
        ...
        chunk_processed = multiprocess_dataframe(chunk_cleaned)
...
def save_final_summary(df, output_file='final_summary.txt'):
    ...
    total_tokens = df['body_token_count'].sum()

    # Combined dictionary counts
    combined_category_counts_total = df['body_category_counts'].apply(pd.Series).sum()

    with open(output_file, 'w') as f:
        f.write(f"Total number of tokens from body: {total_tokens}\n")
        f.write("Total combined category counts for body column:\n")
        f.write(f"{combined_category_counts_total}\n")

    print(f"Final summary saved to {output_file}.")

# Process the dataset in chunks of 100k rows from the CSV file, only reading the 'body' column
processed_df = process_data_in_chunks("../largedata/new_range/parler_posts_comments.csv", chunk_size=100000)

# Save final token count and combined dictionary counts to a text file
save_final_summary(processed_df, output_file='final_summary_parler.txt')
